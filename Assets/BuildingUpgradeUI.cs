...
{
    public GameObject ressourcesCostContainer;
    ...
    void Update()
    {
        ...
        if (this.foodCost.gameObject.activeSelf == false
         && this.woodCost.gameObject.activeSelf == false
         && this.ironCost.gameObject.activeSelf == false
         && this.goldCost.gameObject.activeSelf == false
         && this.manaCost.gameObject.activeSelf == false)
        { //Si toute les ressources sont absentes on libere la place
            this.ressourcesCostContainer.SetActive(false);
            this.ressourcesCostContainer.gameObject.GetComponent<RectTransform>().localScale = new Vector3(0, 0, 0);
        }
        else
        {
            this.ressourcesCostContainer.SetActive(true);
            this.ressourcesCostContainer.gameObject.GetComponent<RectTransform>().localScale = new Vector3(1, 1, 1);
        }
    }
    ...
    {
        upgradeButton.enabled = isActive;
    }
}
